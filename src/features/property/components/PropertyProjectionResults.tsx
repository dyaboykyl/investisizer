...
import { LoadingOverlay, TableSkeleton } from '@/features/shared/components/LoadingStates';
...
export const PropertyProjectionResults: React.FC<PropertyProjectionResultsProps> = observer(({ asset }) => {
  ...
  const { result: computedResults, isLoading } = useComputationLoading(
    ...
    () => asset.results,
    [
      asset.inputs.purchasePrice,
      asset.inputs.downPaymentPercentage,
      asset.inputs.interestRate,
      asset.inputs.loanTerm,
      asset.inputs.monthlyPayment,
      asset.inputs.propertyGrowthRate,
      asset.inputs.propertyGrowthModel,
      asset.inputs.currentEstimatedValue,
      asset.inputs.isRentalProperty,
      asset.inputs.monthlyRent,
      asset.inputs.rentGrowthRate,
      asset.inputs.vacancyRate,
      asset.inputs.maintenanceRate,
      asset.inputs.propertyManagementEnabled,
      asset.inputs.listingFeeRate,
      asset.inputs.monthlyManagementFeeRate,
      JSON.stringify(asset.inputs.saleConfig), // Track sale config changes
      portfolioStore.years,
      portfolioStore.startingYear
    ],
    200 // Minimum loading time for better UX with complex property calculations
  );
    ...
    <LoadingOverlay
      ...
      skeleton={
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-3 md:p-6 mt-6 border border-gray-200 dark:border-gray-700">
          <TableSkeleton rows={parseInt(portfolioStore.years) || 10} columns={6} />
        </div>
      }
    >
